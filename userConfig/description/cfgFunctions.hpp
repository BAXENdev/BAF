
class BAF_UCFG {
    class Loadouts {
        file = "userConfig\loadouts";
        class loadoutConfigWest {};
        // class loadoutConfigEast {};
        // class loadoutConfigInde {};
        // class loadoutConfigCivi {};
    };
};

class CMS { // Custom Mission Scripts
    class MissionScripts {
        file = "userConfig\customMissionScripts";
        // Example definition of function CMS_fnc_function;
        // class function {}; // refers to \userConfig\customMissionScripts\fn_function.sqf (or .fsm).
        // All custom function definitions go below.

    };

    class ZEN_MissionScripts {
        file = "userConfig\customMissionScripts\zen";
        // Example definition of zen function: CMS_fnc_zen_function;
        // class zen_function {}; // refers to \userConfig\customMissionScript\zen\fn_zen_function.sqf.
        // All custom zen function definitions go below.

    };
};
