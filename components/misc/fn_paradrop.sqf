...
if !(isServer) exitWith { _this remoteExec ["BAX_MISC_fnc_paradrop",2]; };
if !(canSuspend) exitWith { _this spawn BAX_MISC_fnc_paradrop; };
...
waitUntil {
	...
	if !(alive _grenade) then {
		_grenadeName createVehicle _position;
		_grenade attachTo [_chute,[0,0,0]];
	};
	position _object select 2 < 0.5 || isNull _chute;
};
detach _object;
