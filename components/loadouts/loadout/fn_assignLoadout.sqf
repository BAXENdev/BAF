...
if (isNil "_selectedVest") then {
	...
	if (count _vestElement isEqualTo 1) then {
		_vestElement set [1,[]];
	};
};
// Random uniform
...
if (isNil "_selectedUniform") then {
	...
	if (count _uniformElement isEqualTo 1) then {
		_uniformElement set [1,[]];
	};
};
// Random backpack
...
if (isNil "_selectedBackpack") then {
	...
	if (count _backpackElement isEqualTo 1) then {
		_backpackElement set [1,[]];
	};
};
// Random goggle
