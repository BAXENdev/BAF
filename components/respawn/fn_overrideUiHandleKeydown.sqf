ace_spectator_fnc_ui_handleKeyDown = {
    params ["","_key","_shift","_ctrl","_alt"];

    if (_key == 0x1F && _ctrl && !(alive player)) exitWith { // CTRL + s : select location
        
        true;
    };

    if (_key == 0x20 && _ctrl && !(alive player)) exitWith { // CTRL + d : select loadout
        // [player] call BAX_LOADOUTS_fnc_dialogSelectLoadout;
        true;
    };

    if (_key == 0x13 && _ctrl && !(alive player)) exitWith { // CTRL + r : respawn
        setPlayerRespawnTime 0; // TODO: Fancy respawn function
        true;
    };
